...
function setup() {
  ...
  function drawHandDrawnCircle(cx, cy, numLayers, maxRadius){
    let outerMostRadius = maxRadius;// fix radius scope
    ...
    //Add colored dots to the outer circle of each circle

    let dot = 20;
    let outDotRadius = outerMostRadius + 10; //put the dot outside the circle
    for (let i = 0; i < dot; i++){
        ...
        noStroke();
        ellipse(x , y, 5, 5);//this is dot size
    }
  }
  ...
  //Refering zigzagcircle code (Jera0420,2024), see appendix.
    ...
    let dots = 20; //put the dots outside the zigzagcircle
